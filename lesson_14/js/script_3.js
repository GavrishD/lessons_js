...
let totalCostNewPrice = dataList.reduce(
    (prevSum, element) => (prevSum += element.price),
    0
)
document.write(`Загальна вартість нових цін: ${totalCostNewPrice}`)


// 2) Знайти кількість товарів, у яких ціна зменшилась (price < old_price)------
let numberProductsPriceDecreased = dataList.filter(
    (element) => element.price < element.old_price
)
console.log(numberProductsPriceDecreased)


// 3) Товари, які доступні (sell_status:"available")----------------------------
let productsAvailable = dataList.filter(
    (element) => element.sell_status === "available"
)
console.log(productsAvailable)


// 4) Сформувати новий список об”єктів тільки доступних для продажу товарів, які 
// міститимуть тільки ідентифікатор товару (id), нову ціну (price), стару ціну 
// (old_price), та ціну у доларах (usd_price)
let newListObjects = productsAvailable.map((element) => {
    return {
        id: element.id,
        price: element.price,
        old_price: element.old_price,
        usd_price: element.usd_price,
    }
})
console.log(newListObjects)